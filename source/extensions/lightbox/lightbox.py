...
class Lightbox(Directive):
    ...
    def run(self):
        ...
        # Determine the source directory and create corresponding path in _static
        source_dir = os.path.dirname(env.doc2path(env.docname, base=False))
        static_image_dir = os.path.join('_static', source_dir)

        # Create the directory if it doesn't exist
        os.makedirs(os.path.join(env.app.outdir, static_image_dir), exist_ok=True)

        # Construct the full source path
        src_image_path = os.path.join(env.srcdir, image_path)

        # Construct the new image path
        new_image_path = os.path.join(static_image_dir, os.path.basename(image_path))
        ...
        try:
            ...
            # Use the original image path if copy fails
            new_image_path = image_path

        image_path = new_image_path.replace(os.path.sep, '/')
        ...
        <label for="{checkbox_id}">
            <img src="{image_path}" alt="{alt_text}" class="lightbox-trigger" style="width: {thumbnail_width};">
        </label>
        <input type="checkbox" id="{checkbox_id}" class="lightbox-toggle">
        <div class="lightbox">
            <label for="{checkbox_id}" class="close-btn">&times;</label>
            <label for="{checkbox_id}">
                <div class="lightbox-content">
                    <img src="{image_path}" alt="{alt_text}" style="width: {lightbox_width};">
...
def setup(app):
    ...
    app.add_css_file(os.path.join(html_dir, 'static', 'lightbox.css'))
